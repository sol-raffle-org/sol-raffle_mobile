...
export const updateFlipGameData = (
  flipGameData?: FlipGameInterface[],
  updatedData?: any,
  kick?: boolean
) => {
  ...
  if (updatedData.status === FlipGameStatusEnum.Created && !kick) {
    ...
    const firstItemEnd = temp.findIndex(item => {
      return (
        Number(item.deleteTime || 0) > 0 &&
        currentTime >= Math.floor((item?.deleteTime || 0) / 1000)
      );
    });
    if (firstItemEnd !== -1) {
  ...
  const newGameData = flipGameData.map(item => {
    const matched =
      item.userCreator.wallet ===
        (updatedData.creator || updatedData?.userCreator?.wallet) &&
    ...
    return matched
      ? {
          ...
          ...updatedData,
          userJoin:
            kick === true ? null : updatedData?.userJoin || item.userJoin,
        }
      : item;
