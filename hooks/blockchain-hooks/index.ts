...
const useTransaction = () => {
  ...
  const [transactionStatus, setTransactionStatus] = useState<BlockchainTransactionStatusEnum | undefined>(undefined)
  ...
  const handleSendTransaction = async (
    ...
    try {
      ...
      if (resTransaction.messageError) {
        ...
        if (!resTransaction.messageError.toLowerCase().includes('declined')) {
  ...
  const handleGetTransactionResult = async (chain: SupportedChainEnum, txHash: string) => {
    try {
      let txStatus = BlockchainTransactionStatusEnum.LOADING

      const rpcEndpoint = getSVMRpcEndpoint(chain)

      txStatus = (await getBlockchainServiceByChain(chain)?.getTransactionResult({
        txHash,
        rpcEndpoint,
