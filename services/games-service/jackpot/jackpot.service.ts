import {
  GET_FAIRNESS_JACKPOT,
  GET_FAIRNESS_JACKPOT_DETAIL,
  GET_USER_JACKPOT_TRANSACTION,
...
export const getUserJackpotHistoryService = async (
  page: number,
  size: number
) => {
  const url = `${GET_USER_JACKPOT_TRANSACTION}?pageNum=${page}&pageSize=${size}`;
  const response: ApiResponse<GetUserJackpotHistoryInterface | undefined> =
...
export const getJackpotFairnessService = async (page: number, size: number) => {
  ...
  const response: ApiResponse<JackpotFairnessInterface | undefined> =
    await createDappServices().get(url);

  return response?.data;
};

export const getJackpotFairnessTransactionDetailService = async (
  roundId: string
) => {
  const url = `${GET_FAIRNESS_JACKPOT_DETAIL}?round=${roundId}`;

  const response: ApiResponse<
    JackpotFairnessTransactionInterface[] | undefined
  > = await createDappServices().get(url);
