...
export const getBetAccountService = async (walletAddress: string) => {
  try {
    ...
    const betAccountPda = web3.PublicKey.findProgramAddressSync(
      [
        Buffer.from("raffle_bet_account"),
...
const createBetAccountInstruction = async (
  walletAddress: string,
  betAccountPda: web3.PublicKey
) => {
  try {
...
export const createPlaceBetTransactionService = async (
  walletAddress: string,
  betAmount: number
) => {
  try {
    ...
    const roundPda = web3.PublicKey.findProgramAddressSync(
      [
        Buffer.from("raffle_round"),
        convertRoundIdToBuffer(gameAccount.currentRoundId.toString()),
      ],
      program.programId
    )[0];

    const betQueuePda = web3.PublicKey.findProgramAddressSync(
      [Buffer.from("raffle_bet_queue")],
      program.programId
    )[0];
    ...
    if (!betAccount) {
      const createBetAccountIx = await createBetAccountInstruction(
        walletAddress,
        betAccountPda
      );
