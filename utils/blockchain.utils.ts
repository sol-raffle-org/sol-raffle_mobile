...
export const getSolanaNativeTokenBalance = async (walletAddress: string, rpcUrl: string) => {
...
export const getSvmSplTokenBalance = async (walletAddress: string, tokenContractAddress: string, rpcUrl: string) => {
  ...
  try {
    ...
    const tokenAmountInfo = await connection.getTokenAccountBalance(tokenAccount[0] || tokenAccount, 'confirmed')
...
export const convertRoundIdToBuffer = (roundId: number) => {
  return Buffer.from(new Uint8Array(new BN(roundId).toArrayLike(Buffer, 'le', 8)))
}

export const getTxHashLink = (txHash: string) => {
  if (!txHash) return '#'

  const params = process.env.NETWORK_MODE !== NetworkModeEnum.MAIN_NET ? `?cluster=${process.env.NETWORK_MODE}` : ''
