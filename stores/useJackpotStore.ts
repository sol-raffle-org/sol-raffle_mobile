...
interface JackpotStore {
  ...
  setCurrentRoundTransaction: (
    value?: JackpotFairnessTransactionInterface[]
  ) => void;
...
const useJackpotStore = createWithEqualityFn<JackpotStore>()(
  persist(
    set => ({
      ...
      setCurrentRoundTransaction: value =>
        set({ currentRoundTransaction: value }),

      // Note: For Jackpot game

      currentAddressBetAccount: undefined,
      setCurrentAddressBetAccount: (value?: BetAccountInfoInterface) =>
        set({ currentAddressBetAccount: value }),

      showWinGlow: false,
      setShowWinGlow: (value: boolean) => set({ showWinGlow: value }),

      gameAccount: undefined,
      setGameAccount: (value?: GameAccountInfoInterface) =>
        set({ gameAccount: value }),

      winnerList: undefined,
      setWinnerList: (value?: JackpotWinnerListInterface) =>
        set({ winnerList: value }),

      jackpotGameData: undefined,
      setJackpotGameData: (value?: JackpotGameDataInterface) =>
        set({ jackpotGameData: value }),
    }),
    {
      name: "jackpot-store",
      partialize: state => ({
